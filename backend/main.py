...
BLINK_THRESHOLD = 0.51  # Restored from 0.65 to original 0.51

# EYE_AR_CONSEC_FRAMES: Number of consecutive frames below the threshold required to confirm a blink
EYE_AR_CONSEC_FRAMES = 2  # Restored from 3 to original 2
...
# Add these constants after other constants, before any function definitions
UPPER_LIP_INDICES = [13, 14, 15, 16, 17]  # Upper lip landmarks
LOWER_LIP_INDICES = [303, 304, 305, 306, 307]  # Lower lip landmarks
MOUTH_CORNER_INDICES = [61, 291]  # Left and right mouth corners
SPEAKING_THRESHOLD = 1.2  # Reduced from 1.5 for better sensitivity
SPEAKING_FRAMES_THRESHOLD = 2  # Reduced from 3 to detect speaking faster
SPEAKING_COOLDOWN_FRAMES = 5  # Frames to maintain speaking state after movement stops
MIN_SPEAKING_MOVEMENT = 0.8  # Minimum movement to consider as potential speech
...
# Add these constants before the main loop starts
SLEEP_THRESHOLD_SECONDS = 3.0  # Time in seconds of closed eyes to consider sleeping
IDLE_THRESHOLD_SECONDS = 3.0   # Time in seconds of no face detection to consider idle
SPEAKING_THRESHOLD = 1.5  # Slightly increased from 1.0 for less sensitivity
SPEAKING_FRAMES_THRESHOLD = 3  # Increased from 2 to require more consistent movement
...
FACE_DETECTION_TIMEOUT = 1.0  # Seconds before considering face lost

# Add these variables for better blink detection
EYES_BLINK_COUNTER = 0
MIN_FRAMES_BETWEEN_BLINKS = 10  # Minimum frames required between blinks to avoid false positives
...
def determine_screen_region(l_dx, l_dy, r_dx, r_dy, pitch, yaw):
    """
    Determine which region of the screen the user is looking at based on gaze and head pose.
    
    Weights:
    - Eye movement: 0.9 (primary factor)
    - Head pose: 0.1 (secondary factor, reduced from previous 0.2)
    
    Thresholds adjusted for more sensitivity to eye movement:
    - Strong movement: > 15 pixels
    - Moderate movement: 8-15 pixels
    - Subtle movement: < 8 pixels
    """
    # Average the left and right eye movements
    avg_dx = (l_dx + r_dx) / 2
    avg_dy = (l_dy + r_dy) / 2
    
    # Weight factors - increased eye dominance
    EYE_WEIGHT = 1
    HEAD_WEIGHT = 0
    
    # Combine head pose with eye movement for more accurate detection
    # Invert pitch for more intuitive vertical movement
    combined_horizontal = (avg_dx * EYE_WEIGHT) + (yaw * HEAD_WEIGHT)
    combined_vertical = (-avg_dy * EYE_WEIGHT) + (-pitch * HEAD_WEIGHT)  # Inverted both components
    
    # Enhanced thresholds for more accurate detection
    STRONG_THRESHOLD = 10
    MODERATE_THRESHOLD = 4
    
    # Determine vertical position (top/bottom) with more sensitivity
    if combined_vertical < -MODERATE_THRESHOLD:
        vertical = "top"
    elif combined_vertical > MODERATE_THRESHOLD:
        vertical = "bottom"
    else:
        vertical = "center"
    
    # Determine horizontal position (left/right) with more sensitivity
    if combined_horizontal < -MODERATE_THRESHOLD:
        horizontal = "left"
    elif combined_horizontal > MODERATE_THRESHOLD:
        horizontal = "right"
    else:
        horizontal = "center"
...
def detect_speaking(mesh_points):
    """
    Detect if the person is speaking based on lip movement.
    Uses both vertical and horizontal lip movements for better accuracy.
    """
    global speaking_frames, is_speaking, prev_lip_distance
    
    # Calculate vertical distances between upper and lower lip points
    vertical_distances = []
    for upper, lower in zip(UPPER_LIP_INDICES, LOWER_LIP_INDICES):
        upper_point = mesh_points[upper]
        lower_point = mesh_points[lower]
        distance = np.linalg.norm(upper_point - lower_point)
        vertical_distances.append(distance)
    
    # Calculate horizontal mouth width (distance between corners)
    left_corner = mesh_points[MOUTH_CORNER_INDICES[0]]
    right_corner = mesh_points[MOUTH_CORNER_INDICES[1]]
    horizontal_distance = np.linalg.norm(right_corner - left_corner)
    
    # Combine vertical and horizontal measurements
    current_lip_state = {
        'vertical': np.mean(vertical_distances),
        'horizontal': horizontal_distance
    }
    
    # Detect movement
    if prev_lip_distance is not None:
        vertical_movement = abs(current_lip_state['vertical'] - prev_lip_distance['vertical'])
        horizontal_movement = abs(current_lip_state['horizontal'] - prev_lip_distance['horizontal'])
        
        # Combined movement score (weighted more towards vertical movement)
        movement = (vertical_movement * 0.7) + (horizontal_movement * 0.3)
        
        if movement > SPEAKING_THRESHOLD and movement > MIN_SPEAKING_MOVEMENT:
            speaking_frames += 1
            if speaking_frames >= SPEAKING_FRAMES_THRESHOLD:
                is_speaking = True
                speaking_frames = SPEAKING_FRAMES_THRESHOLD  # Cap the counter
        else:
            speaking_frames = max(0, speaking_frames - 1)
            if speaking_frames == 0:
                is_speaking = False
    
    prev_lip_distance = current_lip_state
    return is_speaking
...
# Add this function after vector_position and before the main loop starts
def check_presence_and_sleep(face_detected, eyes_aspect_ratio, current_time):
    """
    Check if person is present, idle, sleeping or speaking based on face detection and eye closure.
    """
    global last_face_time, eyes_closed_time, is_speaking
    
    if face_detected:
        last_face_time = current_time
        
        # When eyes_aspect_ratio is HIGH, eyes are CLOSED
        if eyes_aspect_ratio <= BLINK_THRESHOLD:  # Eyes are CLOSED
            if eyes_closed_time is None:
                eyes_closed_time = current_time
            elif current_time - eyes_closed_time >= SLEEP_THRESHOLD_SECONDS:
                return "Inactive"  # Inactive after 3 seconds of closed eyes
            return "Active"  # Still active until 3 seconds pass
        else:  # eyes_aspect_ratio is LOW, eyes are OPEN
            eyes_closed_time = None
            return "Active"
    else:
        if current_time - last_face_time >= IDLE_THRESHOLD_SECONDS:
            eyes_closed_time = None
            return "Idle"
        return "Active"

def draw_gaze_grid(frame, gaze_region):
    """Draw a 3x3 grid showing the current gaze region"""
    h, w = frame.shape[:2]
    cell_w = w // 6
    cell_h = h // 6
    start_x = w - 3 * cell_w - 20  # 20 pixels padding from right
    start_y = 150  # Below the text displays
    
    # Draw the 3x3 grid
    for i in range(4):
        # Vertical lines
        cv.line(frame, 
                (start_x + i * cell_w, start_y), 
                (start_x + i * cell_w, start_y + 3 * cell_h),
                (128, 128, 128), 1)
        # Horizontal lines
        cv.line(frame,
                (start_x, start_y + i * cell_h),
                (start_x + 3 * cell_w, start_y + i * cell_h),
                (128, 128, 128), 1)
    ...
    if gaze_region in region_map:
        col, row = region_map[gaze_region]
...
try:
    ...
    while True:
        ...
        eyes_aspect_ratio = 1.0  # Default value when no face is detected
        gaze_region = "center_center"  # Default gaze region
        ...
        if face_detected:
            ...
            # Get the 3D landmarks from facemesh x, y and z
            mesh_points_3D = np.array(
                [[n.x, n.y, n.z] for n in results.multi_face_landmarks[0].landmark]
            )
            
            # Calculate eyes_aspect_ratio first
            eyes_aspect_ratio = blinking_ratio(mesh_points_3D)
            
            # Add blink detection logic
            if not hasattr(check_presence_and_sleep, "frames_since_last_blink"):
                check_presence_and_sleep.frames_since_last_blink = MIN_FRAMES_BETWEEN_BLINKS
            
            # checking if ear less than or equal to required threshold
            if eyes_aspect_ratio <= BLINK_THRESHOLD:
                EYES_BLINK_FRAME_COUNTER += 1
            else:
                # If eyes were closed for enough frames and enough frames have passed since last blink
                if EYES_BLINK_FRAME_COUNTER >= EYE_AR_CONSEC_FRAMES and check_presence_and_sleep.frames_since_last_blink >= MIN_FRAMES_BETWEEN_BLINKS:
                    TOTAL_BLINKS += 1
                    check_presence_and_sleep.frames_since_last_blink = 0
                EYES_BLINK_FRAME_COUNTER = 0
            
            # Increment frames since last blink
            check_presence_and_sleep.frames_since_last_blink += 1

            # Process and calculate eye features first
            ...
            # Calculate relative positions early
            ...
            gaze_region = determine_screen_region(l_dx, l_dy, r_dx, r_dy, angle_x, angle_y)
            ...
            if SHOW_ON_SCREEN_DATA:
                ...
                gaze_color = (255, 165, 0)  # Orange color for gaze
                cv.putText(
                    frame,
                    f"Gaze Region: {gaze_region.replace('_', ' ').title()}",
                    (img_w - 400, 120),
                    cv.FONT_HERSHEY_TRIPLEX,
                    0.8,
                    gaze_color,
                ...
                # Draw the gaze grid
                draw_gaze_grid(frame, gaze_region)
            ...
            # Display facial landmarks if enabled
            if SHOW_ALL_FEATURES:
                for point in mesh_points:
                    cv.circle(frame, tuple(point), 1, (0, 255, 0), -1)

            # Highlight eye features
        ...
        presence_status = check_presence_and_sleep(face_detected, eyes_aspect_ratio, current_time)
        ...
        if SHOW_ON_SCREEN_DATA:
            ...
            status_color = (0, 255, 0)  # Default green for Active
            if face_detected:
                is_talking = detect_speaking(mesh_points)
                
                if presence_status == "Inactive":
                    status_color = (0, 0, 255)  # Red for Inactive
                elif presence_status == "Idle":
                    status_color = (0, 165, 255)  # Orange for Idle
                
                status_text = f"Status: {presence_status}"
                if is_talking:
            ...
            cv.putText(frame, status_text, (30, 260),
